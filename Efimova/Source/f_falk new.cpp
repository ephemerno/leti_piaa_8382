   ...
   while (!stak.empty()) {       //обработка, пока стек не пуст
        ...
        for( int j = 0 ; j < U; j++){
            if(Graph[i][j] > 0 && visited[j] == false){
                ...
                parent[j] = i;
                visited[j] = true;
                cout << "\tAdd new incident top in stack " << node[j] << endl;
            }
        }
        cout << "The end of dfs" << endl;
    }
    if(visited[t] == true){
...
int f_Fulkerson(vector<vector<int>>& graph, vector<vector<int>>& Graph, int s, int t, int U, string node) {
	...
	while (dfs(Graph, s, t, parent, U, node)) {  
		...
		cout << "The flow is : " << path_flow << endl;
	...
	cout << "The end of f_Fulkerson" << endl;
	return max_flow;
}
